...
export async function POST(request: NextRequest) {
  ...
  console.log(`[r2-confirm:${requestId}] ═══════════════════════════════════════════════════════`);
  console.log(`[r2-confirm:${requestId}] POST /api/portals/r2-confirm (called by Worker)`);

  try {
    const body = await request.json();
    console.log(`[r2-confirm:${requestId}] Request body:`, JSON.stringify(body, null, 2));
    ...
    console.log(`[r2-confirm:${requestId}] workerSecret provided: ${!!workerSecret} (length: ${workerSecret?.length})`);
    console.log(`[r2-confirm:${requestId}] env WORKER_SECRET present: ${!!process.env.WORKER_SECRET} (length: ${process.env.WORKER_SECRET?.length})`);

    if (!workerSecret || workerSecret !== process.env.WORKER_SECRET) {
      console.error(`[r2-confirm:${requestId}] ❌ Unauthorized: worker secret mismatch`);
    ...
    if (status === "completed") {
      ...
      console.log(`[r2-confirm:${requestId}] clientSessionId: ${clientSessionId}`);

      if (resolvedSessionId) {
        console.log(`[r2-confirm:${requestId}] Updating existing UploadSession: ${resolvedSessionId}`);
        ...
        console.log(`[r2-confirm:${requestId}] ✓ UploadSession created: ${resolvedSessionId}`);
      }
      ...
      console.log(`[r2-confirm:${requestId}] ✓ File record created: ${file.id}`);

      // ── Update staging record with fileId ─────────────────────────────────
      console.log(`[r2-confirm:${requestId}] Updating R2StagingUpload to completed...`);
      ...
      if (!skipNotification) {
        ...
        try {
          ...
          if (portal) {
            console.log(`[r2-confirm:${requestId}] Portal found for notification: ${portal.name}`);
            ...
            console.warn(`[r2-confirm:${requestId}] ⚠️ Portal not found for notification`);
          }
        } catch (emailErr) {
          console.error(`[r2-confirm:${requestId}] ❌ Email notification failed (non-fatal):`, emailErr);
        }
      } else {
        console.log(`[r2-confirm:${requestId}] Skipping email notification (skipNotification=true)`);
      }

      console.log(`[r2-confirm:${requestId}] ✓✓✓ SUCCESS - Returning file data`);
      console.log(`[r2-confirm:${requestId}] ═══════════════════════════════════════════════════════`);
      return NextResponse.json({
        success: true,
        fileId: file.id,
    ...
    if (status === "failed") {
      console.error(`[r2-confirm:${requestId}] ❌ Worker reported FAILED status`);
      console.error(`[r2-confirm:${requestId}] Transfer error:`, transferError);

      console.log(`[r2-confirm:${requestId}] Updating R2StagingUpload to failed...`);
      await prisma.r2StagingUpload.update({
        where: { stagingKey },
        data: { status: "failed" },
      });
      console.log(`[r2-confirm:${requestId}] ✓ R2StagingUpload marked as failed`);

      console.error(`[r2-confirm:${requestId}] Transfer failed for stagingKey=${stagingKey}:`, transferError);
      console.log(`[r2-confirm:${requestId}] ═══════════════════════════════════════════════════════`);
    ...
    console.error(`[r2-confirm:${requestId}] Error stack:`, error instanceof Error ? error.stack : "N/A");
    console.error(`[r2-confirm:${requestId}] ═══════════════════════════════════════════════════════`);
